...
import click
import requests
from analysis import count_tags, filter_tags_by_date_range
...
class BeeminderGoal:
    ...
    def post_datapoint(self, daystamp, value):
        url = 'https://www.beeminder.com/api/v1/users/{}/goals/{}/datapoints.json'.format(
            self.user, self.goal)
        params = {'daystamp': daystamp, 'value': value,
...
def get_daily_count(tags):
    ...
    begin_day = dt.date(year=begin_dt.year,
                        month=begin_dt.month, day=begin_dt.day)
...
@click.argument('goal')
@click.option('-c', '--config', 'configfile', help='path to config', type=click.Path(exists=True, readable=True, resolve_path=True), default=os.path.expanduser('~/.bmndrrc'))
@click.option('--tags', 'synctags', default='', help='comma seperated list of tags')
@click.option('--gap', 'gap', default=0.75, help='average interval between tags')
@click.option('--update', 'update', is_flag=True, help='if set update existing datapoints')
def sync(path, goal, synctags, gap, update,configfile):
